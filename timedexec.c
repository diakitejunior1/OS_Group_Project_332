...
void handle_sigint(int sig){
    printf("\n[Signal] Exiting program by SIGINT...\n");
    exit(0);
}

void handle_sigtstp(int sig){
    stop = 1;
    printf("\n[Signal] Pausing monitoring loop. Use 'fg' to resume.\n");
    fflush(stdout);
}

void handle_sigcont(int sig){
    stop = 0;
    printf("[Signal] Continuing monitoring.\n");
    fflush(stdout);
}
...
//strcmp to confirm if actual arguement matches with the name of arguements
    ...
    long max_mem_used = 0; // Tracks the peak RSS seen so far
